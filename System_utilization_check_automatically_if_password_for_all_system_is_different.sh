...
# ============================================================
#  SYSTEM + GPU MONITOR — SSH-based  (per-system passwords)
# ============================================================
#
#  SYSTEMS map format:
...
monitor_system() {
    ...
    local ip=$1
    local user=$2
    local password=$3        # <── per-system password
...
'
    ...
    # ── SSH using the per-system password ──────────────────────
    ...
    if [ "$has_gpu" -eq 1 ] && [ "$gpu_count" -gt 0 ]; then
        ...
        for ((gid=0; gid<gpu_count; gid++)); do
            ...
            if [ "$g_count" -gt 0 ]; then
                ...
                echo "${user}|${ip}|GPU_${gid}|${avg_cpu}|${avg_mem_used}/${mem_total_val}GB|${avg_mem_pct}GB|${avg_gutil}|${avg_gmu}/${g_mt}GB|${avg_gtemp}°C" \
                    >> "$TABLE_FILE"
        ...
        echo "${user}|${ip}|NO_GPU|${avg_cpu}|${avg_mem_used}/${mem_total_val}GB|${avg_mem_pct}GB|N/A|N/A|N/A" \
            >> "$TABLE_FILE"
...
for ip in "${!SYSTEMS[@]}"; do
    ...
    # Split "username:password" stored in the map value
    IFS=':' read -r USERNAME PASSWORD <<< "${SYSTEMS[$ip]}"

    echo "----------------------------------------" | tee -a "$OUTPUT_FILE"
    echo "Monitoring : $USERNAME@$ip" | tee -a "$OUTPUT_FILE"
    echo "----------------------------------------" | tee -a "$OUTPUT_FILE"

    if monitor_system "$ip" "$USERNAME" "$PASSWORD"; then
        ((SUCCESS++))
    else
        ((FAIL++))
    fi
